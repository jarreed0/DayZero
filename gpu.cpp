...
void write(std::string t, int x, int y) {
 ...
 GPU_BlitRect(text_texture, NULL, &wrect, renderer);
...
void drawRectUpfront(SDL_Rect r, SDL_Color c) {
 ...
 GPU_RectangleFilled(renderer, &r);
}
...
void init() {
 ...
 WIDTH=DM.w;
 HEIGHT=DM.h;
 window = GPU_SetInitWindow(TITLE, SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, WIDTH, HEIGHT, SDL_WINDOW_SHOWN);
 renderer = GPU_Init(window, -1, SDL_RENDERER_ACCELERATED | SDL_RENDERER_PRESENTVSYNC);
 GPU_SetShapeBlendMode(renderer, SDL_BLENDMODE_BLEND);
