...
void floorPer() {
 ...
 for(int y = 0; y < map_height; y++) {
  for(int x = 0; x < map_width; x++) {
   ...
   } else if(map[y*map_width + x].id == FLOOR && rand() % 1200 == 5) { dropChess(x, y);
...
void fireBullet(int id, double sx, double sy, double a, double type, double sw, double v, double dmg, bool bnc) {
    ...
    bullet.loc.x += sw * cos(degToRad(a));
    bullet.loc.y += sw * sin(degToRad(a));
    bullet.extra = dmg;
    bullet.bExtra = bnc;
    bullet.tick=800;
...
void updateBullets() {
    ...
    for(int b=0; b<bullets.size(); b++) {
        bullets[b].loc.x += bullets[b].vel * cos(degToRad(bullets[b].angle));
        bullets[b].loc.y += bullets[b].vel * sin(degToRad(bullets[b].angle));
        bullets[b].tick--;
        for(auto m:map) {
        ...
        if(SDL_HasIntersection(&player.loc, &bullets[b].loc)) {
            ...
            if(collectables[c].id < 5) {
                ...
                if(rand() % 2) {
                    ...
                    while(unlock) {
                        ...
                        if(stuck) {
                            for(int i=0; i<6; i++) {
                                if(!modsUnlocked[i]) {modsUnlocked[i]=1; mod=u;i=1000;}
                            }
                        }
                    }
                } else {
                    ...
                    while(unlock) {
                        ...
                        if(stuck) {
                            for(int i=0; i<5; i++) {
                                if(!mods2Unlocked[i]) {mods2Unlocked[i]=1; mod2=u;i=1000;}
                            }
                        }
                    }
                }
